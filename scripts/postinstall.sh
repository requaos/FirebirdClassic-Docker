...
INSTALLDIR=`pwd`		# Added by Evolution -- SJF 2016-04-03
...
# changeDBAPassword
    ...
    #----------------------------------- Evo changes --------------------------
    # We do both SYSDBA and EUSER here, and since we're running as root we do
    # not need the *old* sysdba password (which is often masterkey). 
    # 
    # we want to (but do not yet) actually use the DEFAULT_* stuff, allowing the
    # user to accept the defaults. This may be that we need to write our own
    # AskQuestion
    #
AskPassword() {
    Test=$1
    ...
    echo -n "${1}"
    Answer="$DefaultAns"
    ## Using defaults always by commenting this out below
    #read Answer

    if [ -z "$Answer" ]
    then
        Answer="$DefaultAns"
echo "Using default of \"$DefaultAns\""
    fi
}

    DEFAULT_SYSDBA=pps97
    DEFAULT_EUSER=pps97

    ## Parse evo-config?
    if [ -e /usr/local/bin/evo-config ]
    then
	echo "Loading evo-config"
    fi

    NewPasswd=""
    while [ -z "$NewPasswd" ]
    do
        AskPassword "Please enter new password for SYSDBA user : " "$DEFAULT_SYSDBA"
        NewPasswd=$Answer
        if [ ! -z "$NewPasswd" ]
        then
            if ! runSilent "/opt/firebird/bin/gsec -modify SYSDBA -pw $NewPasswd"
            then
		NewPasswd=""
	    fi
        fi
    ...
    /opt/firebird/bin/gsec -delete EUSER 2>/dev/null

    NewPasswd=""
    while [ -z "$NewPasswd" ]
    do
        AskPassword "Please enter new password for EUSER  user : " "$DEFAULT_EUSER"
        NewPasswd=$Answer
        if [ ! -z "$NewPasswd" ]
        then
            if ! runSilent "/opt/firebird/bin/gsec -add EUSER -pw $NewPasswd"
            then
		NewPasswd=""
	    fi
	fi
    ...
    # 2016-10-14; this was here but just now documented. We really do require
    # /db/dbtemp so root or /tmp doesn't run out of space

    if [ -d /db ]  &&  [ ! -d /db/dbtemp ]
    then
        echo "Creating /db/dbtemp"
        mkdir --mode 777 /db/dbtemp
    fi

    # 2016-10-20: if the directory is there but has the wrong mode,
    # fix it. Things break badly when the temp area isn't writable

    [ -d /db/dbtemp ]  &&  chmod 777 /db/dbtemp

    # added 2015/02/03 -- SJF
    if [ -d /db ]  &&  [ ! -d /db/evolution ]
    then
	echo "Creating /db/evolution"
	mkdir /db/evolution
	chown firebird:firebird /db/evolution
    fi

    # make sure we can edit this file
    chmod u+w /opt/firebird/firebird.conf

    # Enable and start xinetd, make sure ntpd is running too
    ...
    if [ -d /etc/sysctl.d ]
    then
        cp $INSTALLDIR/misc/evo-sysctl.conf /etc/sysctl.d/.
    else
        : no sysctl to install?
    fi

    echo ""
    echo "Patching files: these might show failure if they have"
    echo "already been applied; this is OK"
    echo ""

    # run all our patches
    while read file patchfile junk
    do

	if [ -e $file ]
	then
		echo "Patching $file"
		# --forward: ignore patches that appear already applied
		...
		patch --reject-file=- \
			...
			--silent \
			--strip=0 \
			--input=$INSTALLDIR/misc/$patchfile $file
	fi

    done <<EOF
/etc/security/limits.d/20-nproc.conf 20-nproc.conf.patch	# CentOS 7
/etc/security/limits.d/90-nproc.conf 90-nproc.conf.patch	# CentOS 6
EOF
## don't patch this; we're sticking with process
## /opt/firebird/bin/changeMultiConnectMode.sh	changeMultiConnectMode.sh.patch

#/etc/profile                         etc.profile.patch

    FBPROFILE=/etc/profile.d/firebird-path.sh

    # Add /etc/profile.d/firebird-path.sh if needed
    if [ ! -e $FBPROFILE ]
    then
        echo "Installing $FBPROFILE"
        cp $INSTALLDIR/misc/firebird-path.sh $FBPROFILE
    else
        echo "$FBPROFILE already exists; not overwriting"
    fi
    ...
    ## TODO: Make sure Firebird port is open
    ## firewall-cmd --zone=public --add-port=3050/tcp --permanent
    ## firewall-cmd --reload

    echo "You'll usually answer 'process' to this question"
    echo ""
    echo ""

    # TODO: force it to take a cmd line param?
    /opt/firebird/bin/changeMultiConnectMode.sh
 
    #----------------------------------- Evo changes --------------------------

### SJF 2016-04-03
echo "Leave postinstall.sh"
