#!/bin/sh
#
# $Id: //devel/isystems/fb-installer/main/2.5.5cs/extra/install.sh#1 $

echo
echo "Installing Firebird-2.5.5 x64 customized for Evolution"
echo

#----------------------------- Evo customizations ---------------------------------

# TODO: is this even the case? We can probably run this on any version?
#
#if [ ! -d /usr/lib/systemd ]
#then
#	echo "ERROR: This installer works only for systemd-based Linux (CentOS 7)"
#	echo "  For CentOS 5 or 6, use the standard RPM installer"
#
#	exit 1
#fi
...
# Requirements for Firebird 2.5.5 install
# From the firebirdsql.org download site
#
...
# If your glibc version is lower than v.2.7, it must be upgraded before using Firebird 2.5 Classic/Super?lassic.

case `uname --machine` in
  x86_64)
	echo "This is a 64-bit machine; good."
	;;

  *)
	echo "ERROR: This does not appear to be a 64-bit machine; no way it works with Evo"
	exit 1
	;;
esac


KERNELVER=`uname --release`

KERNELVER=`echo $KERNELVER | sed s/-.*$//`

echo "You're running the $KERNELVER Linux kernel"

case "x$KERNELVER" in
  x3.*)
	echo "Kernel 3.x is fine"
	ok=1
	;;

  x4.*)
	echo "Kernel 4.x is unusual but fine"
	ok=1
	;;

  x2.4.*)
	echo "Linux kernel 2.4 is MUCH too old"
	exit 1
	;;

  x2.6.*)

	lastdigit=`expr match $KERNELVER '.*\.\([0-9]\+\)$'`

	if [ -z "$lastdigit" ]
	then
		echo "ERROR: cannot determine last digit of 2.6 kernel version"
		exit 1
	fi

	if [ $lastdigit -ge 34 ]
	then
		echo "Your kernel is fine"

	elif [ $lastdigit -ge 27 ]
	then
		echo "Your kernel is $KERNELVER but 2.6.34 is recommended"

	else 
		echo "ERROR: Your system has kernel version $KERNELVER, but 2.27 is the minimum required"
		exit 1
	fi

	echo "Linux kernel $KERNELVER is ok"
	;;

esac

# Now check GLIBC: must be 2.7 or later

GLIBCVER=`rpm -qa --queryformat "%{VERSION}\n"  glibc | uniq`

case x"$GLIBCVER" in
  x1.*)
	echo "ERROR: This system has GLIBC $GLIBCVER, 2.7 required"
	exit 1
	;;
  x2.*)
	lastdigit=`expr match $GLIBCVER '.*\.\([0-9]\+\)$'`
	...
	echo "GLIBC $GLIBCVER is OK"
	;;
  x*)
	echo "ERROR: Don't know what '$GLIBCVER' is"
	exit 1
	;;
esac

# make sure we don't have an existing Firebird RPM

echo "Checking for existing FirebirdCS RPM install"

if rpm -q FirebirdCS
then
	echo "Firebird is installed by RPM; please uninstall first"
	exit 1
fi


# TODO: insure we have a suitable version?
# TODO: Key off of CentOS 6 or 7?

# We have a handful of packages we require, either for installation, or
# at least are nice to have.

PKGLIST=compat-libstdc++-33		# Firebird absolutely won't work without this
PKGLIST="$PKGLIST wget"			# we use this way too often not to have around
PKGLIST="$PKGLIST xinetd"		# required for Firebird to listen to the network
PKGLIST="$PKGLIST bzip2 zip unzip"	# common compression tools
PKGLIST="$PKGLIST patch"		# required by this installer
PKGLIST="$PKGLIST ntp ntpdate"		# keeping time synchronized
PKGLIST="$PKGLIST lsof"			# good debugging tool
PKGLIST="$PKGLIST net-tools"		# ifconfig netstat and others
PKGLIST="$PKGLIST psmisc"		# fuser and friends

# Add packages 

MISSINGPACKAGES=`rpm -q $PKGLIST | grep '^package' | cut -f2 -d' '  `

if [ ! -z "$MISSINGPACKAGES" ]
then
        echo "Missing necessary packages: $MISSINGPACKAGES"

	echo
	echo -n "Press ENTER to install them (control-C to abandon): "

	read xx

	yum --assumeyes install $MISSINGPACKAGES
fi

# Make sure the system time is OK during installation
# (NTPD is started later later)

ntpdate -s pool.ntp.org 	# -s: silent, don't object if ntpd is running
...
# Insure that EvolUDFs get logged -- 2016/10/10
...
if [ -e scripts ]; then
    ...
    \cp -rf ./scripts/changeMultiConnectMode.sh /opt/firebird/bin/changeMultiConnectMode.sh
    chmod +x /opt/firebird/bin/changeMultiConnectMode.sh
